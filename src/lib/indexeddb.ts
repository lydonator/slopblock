...
interface CDNDeltaResponse {
  metadata: {
    generated_at: string;
    since: string;
    total_updates?: number; // Optional - Edge Function might use this name
    video_count?: number;   // Optional - Database function uses this name
  };
  videos: MarkedVideo[];
}
...
function isValidISO8601(timestamp: any): boolean {
  ...
  if (typeof timestamp === 'string') {
    ...
    // Also accepts timezone offsets like +00:00 or -05:00 in addition to Z
...
function validateDeltaResponse(data: any): asserts data is CDNDeltaResponse {
  ...
  // Validate video count if present (handles both field names)
  // Edge Function might use total_updates, database function uses video_count
  const videoCount = data.metadata.video_count ?? data.metadata.total_updates;

  if (videoCount !== undefined) {
    if (!Number.isInteger(videoCount) || videoCount < 0) {
      throw new CDNValidationError('Invalid metadata video count: must be non-negative integer', {
        video_count: data.metadata.video_count,
    ...
    if (data.videos.length !== videoCount) {
      console.warn(
        `[SlopBlock] Delta metadata count mismatch: expected ${videoCount}, got ${data.videos.length} videos. Using actual array length.`
      );
    }
  }
 ...
 * Delta is pre-generated by cron and stored in CDN - no query parameters needed
 */
export async function syncDelta(deltaUrl: string): Promise<void> {
  try {
    console.log(`[SlopBlock] Fetching delta from CDN: ${deltaUrl}`);
